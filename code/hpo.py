...
#    hook.set_mode(smd.modes.EVAL)
    ...
    with torch.no_grad():
        ...
        print('Test Precision: {test_acc.avg:.3f}'.format(test_acc=test_acc))
        logger.info('Testing Precision: {test_acc.avg:.3f}'.format(test_acc=test_acc))
        print('Average Test loss: {loss.val:.4f}'.format(loss=losses))
        logger.info('Average Test loss: {loss.val:.4f}'.format( loss=losses))
        if file_out==True:
            f.close()

        test_acc_list.append(test_acc.avg)

def train(model, train_loader, validation_loader, criterion, optimizer, epochs, device, hook, file_out):
    global best_prec
    image_dataset={'train':train_loader, 'valid':validation_loader}

    snapshot_fname = model_dir+"snapshots/resnet34.pth.tar"
    snapshot_best_fname = model_dir+"snapshots/resnet34_best.pth.tar"
...
def net(checkpoint_path, device):
    if(checkpoint_path != None and device == 'cuda:0'):
        # create model
...
def main(args):
    ...
    global  best_prec, train_loss_list, val_acc_list, test_acc_list, learning_rate, learning_rate_decay, model_dir
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    epochs = args.epochs
    model_dir = args.model_dir
    checkpoint_path = args.data + 'models/resnet34_best.pth.tar'
...
#                                weight_decay=1e-4)
    ...
    model=train(model, train_loader, validation_loader, criterion, optimizer, epochs, device, hook, True)
